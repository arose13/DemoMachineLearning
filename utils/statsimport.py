...
def plot_surfaces(func, x1_range, x2_range, step=1/100, f_params=None, thres=False, xlabel='', ylabel='', cmap='inferno'):
    xx1, xx2 = np.meshgrid(np.arange(x1_range[0], x1_range[1], step), np.arange(x2_range[0], x2_range[1], step))
    yy = []
    for x_1, x_2, in zip(xx1.ravel(), xx2.ravel()):
        if f_params is not None:
            yy.append(func([x_1, x_2], *f_params))
        else:
            yy.append(func([x_1, x_2]))
    yy = np.array(yy).reshape(xx1.shape)

    fig = graph.figure(figsize=(5, 5))
    ax = fig.gca(projection='3d')
    surface = ax.plot_surface(xx1, xx2, yy, cmap=cmap)
    graph.ylabel(ylabel)
    graph.xlabel(xlabel)
    graph.show()
...
if __name__ == '__main__':
    ...
    def f(x): return np.dot(w, np.array(x).T) + b

    plot_surfaces(f, [-2, 2], [-2, 2], step=0.1)
    plot_surfaces(f, [-1, 1], [-1, 1], xlabel='$x_1$', ylabel='$x_2$')
